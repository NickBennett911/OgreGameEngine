...
#define INPUT_MANAGER ssuge::InputManager::get_singleton_ptr()
...
namespace ssuge
{
	...
	// @@@@@@ ATTRIBUTES @@@@@@ //
		...
		std::set<ssuge::InputListener*> mListeners;
	...
	public:
		...
		void register_listener(InputListener* gobj);

		/// Deregisters a listener (no effect if that listener is not in our set of listeners)
		void deregister_listener(InputListener* gobj);
	...
	protected:
		...
		void handle_action_key(SDL_Keycode k, bool is_pressed);

		/// Our input event processing will call this when a key goes down (passing true) or up (passing false)
		void handle_axis_key(SDL_Keycode k, bool is_pressed);
